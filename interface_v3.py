...
#                                 if stable_count >= 3: # CHỐT KHI ỔN ĐỊNH 3 LẦN
...
class LCRApp(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('QC Wayne Kerr 4300 - Real Inductor Measurement')
        ...
        conn_group = QGroupBox("Điều Khiển Kết Nối")
        conn_group.setStyleSheet("QGroupBox { font-weight: bold; border: 1px solid #ccc; border-radius: 8px; margin-top: 15px; padding-top: 15px; }")
        conn_layout = QHBoxLayout(conn_group)
        self.combo_ports = QComboBox()
        self.btn_connect = QPushButton('KẾT NỐI')
        self.btn_connect.clicked.connect(self.toggle_connection)
        conn_layout.addWidget(QLabel('Cổng COM:'))
        conn_layout.addWidget(self.combo_ports, 1)
        conn_layout.addWidget(self.btn_connect)
        left_layout.addWidget(conn_group)
        ...
        # Khởi tạo các ô rỗng để tránh lỗi NoneType
        for i in range(125):
            for j in range(11):
                it = QTableWidgetItem("")
                if j == 0: it.setText(str(i+1))
        ...
        self.btn_r.clicked.connect(self.auto_measure_r)
        btn_layout.addWidget(self.btn_lq); btn_layout.addWidget(self.btn_r)
        left_layout.addLayout(btn_layout)

        self.lbl_msg = QLabel('HỆ THỐNG SẴN SÀNG')
        self.lbl_msg.setAlignment(Qt.AlignCenter)
        self.lbl_msg.setStyleSheet("color: red; font-weight: bold; font-size: 28pt; padding: 10px;")
        left_layout.addWidget(self.lbl_msg)

        # Raw Log Box
        self.log_box = QTextEdit()
        self.log_box.setFixedHeight(60); self.log_box.setReadOnly(True)
        self.log_box.setStyleSheet("background-color: white; color: black; font-family: Consolas;")
        left_layout.addWidget(self.log_box)

        self.btn_export = QPushButton('XUẤT BÁO CÁO CSV')
        self.btn_export.setFixedHeight(40); self.btn_export.setStyleSheet("background-color: #ef6c00; color: white; font-weight: bold;")
        self.btn_export.clicked.connect(self.export_csv)
        left_layout.addWidget(self.btn_export)

        # PHẦN BÊN PHẢI (35%)
        right_container = QGroupBox("Phân Tích Chất Lượng")
        right_container.setStyleSheet("QGroupBox { font-weight: bold; border: 2px solid #ccc; border-radius: 12px; background-color: white; margin-top: 15px; padding-top: 25px; }")
        right_layout = QVBoxLayout(right_container)
        
        self.lbl_chart_title = QLabel("TỶ LỆ %"); self.lbl_chart_title.setAlignment(Qt.AlignCenter)
        self.lbl_chart_title.setStyleSheet("font-size: 18pt; font-weight: bold;"); right_layout.addWidget(self.lbl_chart_title)

        self.chart = MplsChart(self); right_layout.addWidget(self.chart, 5)

        self.lbl_p = QLabel("PASS: 0"); self.lbl_f = QLabel("FAIL: 0"); self.lbl_w = QLabel("WARNING: 0")
        for lbl, color in zip([self.lbl_p, self.lbl_f, self.lbl_w], ['#4CAF50', '#F44336', '#FFEB3B']):
            lbl.setFixedHeight(50); lbl.setAlignment(Qt.AlignCenter)
            lbl.setStyleSheet(f"background-color: {color}; color: {'white' if color != '#FFEB3B' else 'black'}; font-weight: bold; font-size: 14pt; border-radius: 8px;")
            right_layout.addWidget(lbl)
    ...
    def evaluate_lq(self, val_l, val_q, is_z=False):
        q_limit = self.Q_Z_MIN if is_z else self.Q_XY_MIN
        # FAIL
        if val_l < self.L_MIN or val_l > self.L_MAX or val_q < q_limit:
            return "FAIL", QColor(244, 67, 54)
        # WARNING (15% biên)
        margin = (self.L_MAX - self.L_MIN) * 0.15
        if (val_l <= self.L_MIN + margin) or (val_l >= self.L_MAX - margin):
            return "WARNING", QColor(255, 235, 59)
        # PASS
        return "PASS", QColor(76, 175, 80)

    def auto_measure_lq(self):
        if not self.ser: QMessageBox.warning(self, "Lỗi", "Chưa kết nối!"); return
        self.ser.write(b":MEASure:FUNC1 L\r\n:MEASure:FUNC2 Q\r\n") # Cài đặt chế độ đo L-Q [cite: 1999, 2000]
        
        for row in range(125):
            self.table.selectRow(row)
            row_evals = []
            for pair_idx, axis in zip([1, 3, 5], ['LX', 'LY', 'LZ']):
                self.lbl_msg.setText(f"LK {row+1}: CHỜ ĐO {axis}...")
                stb_count, last_l = 0, 0
                
                while True:
                    self.ser.write(b":MEASure:TRIGger\r\n") # Kích hoạt đo [cite: 1899]
                    time.sleep(0.15)
                    self.ser.write(b":MEASure:RESult?\r\n") # Lấy kết quả [cite: 1915]
                    line = self.ser.readline().decode('ascii').strip()
                    if line: self.log_box.append(line)
                    
                    if ',' in line:
                        try:
                            parts = line.split(',')
                            v_l_mh = float(parts[0]) * 1000 # Henry -> mHenry
                            v_q = float(parts[1])

                            # LOGIC LỌC 
                            if (self.L_MIN * 0.5) <= v_l_mh <= (self.L_MAX * 1.5):
                                if abs(v_l_mh - last_l) < (self.L_MAX - self.L_MIN) * 0.01: stb_count += 1
                                else: stb_count = 0
                                last_l = v_l_mh
                                
                                if stb_count >= 2:
                                    st, color = self.evaluate_lq(v_l_mh, v_q, is_z=(pair_idx==5))
                                    self.table.item(row, pair_idx).setText(f"{v_l_mh:.3f}")
                                    self.table.item(row, pair_idx).setBackground(color)
                                    self.table.item(row, pair_idx+1).setText(f"{v_q:.2f}")
                                    self.table.item(row, pair_idx+1).setBackground(color)
                                    row_evals.append(st); break
                        except: pass
                    QApplication.processEvents()

                self.lbl_msg.setText(f"LK {row+1}: NHẤC KIM OK.")
                while True:
                    self.ser.write(b":MEASure:RESult?\r\n")
                    res = self.ser.readline().decode('ascii').strip()
                    try:
                        if res and ',' in res:
                            v_check = float(res.split(',')[0]) * 1000
                            if v_check > self.L_MAX * 5 or v_check < self.L_MIN * 0.1: break
                    except: break
                    time.sleep(0.1); QApplication.processEvents()

            final = "FAIL" if "FAIL" in row_evals else ("WARNING" if "WARNING" in row_evals else "PASS")
            self.table.item(row, 10).setText(final)
            self.table.item(row, 10).setBackground(QColor(76,175,80) if final=="PASS" else (QColor(244,67,54) if final=="FAIL" else QColor(255,235,59)))
            self.update_chart(self.get_statistics())
            
        self.lbl_msg.setText("HOÀN THÀNH 125 LK!")

    def auto_measure_r(self):
        if not self.ser: return
        self.ser.write(b":MEASure:FUNC1 R\r\n") # Cài đặt đo R [cite: 1999]
        specs = [self.RX_SPECS, self.RY_SPECS, self.RZ_SPECS]
        
        for row in range(125):
            self.table.selectRow(row)
            for i, col in enumerate([7, 8, 9]):
                self.lbl_msg.setText(f"LK {row+1}: ĐO R {['X','Y','Z'][i]}...")
                ...
                while True:
                    ...
                    try:
                        if ',' in line:
                            r = float(line.split(',')[0])
                            if r < 1000000: # Lọc hở mạch 
                                st, color = ("PASS", QColor(204, 255, 204)) if min_r <= r <= max_r else ("FAIL", QColor(255, 204, 204))
                                self.table.item(row, col).setText(f"{r:.2f}")
                                self.table.item(row, col).setBackground(color); break
                    except: pass
                    QApplication.processEvents()
            time.sleep(0.1)
    ...
    def update_chart(self, stats):
        ...
        self.chart.axes.pie(stats if sum(stats) > 0 else [1], colors=['#4CAF50', '#F44336', '#FFEB3B'] if sum(stats) > 0 else ['#eee'], startangle=90, wedgeprops=dict(width=0.45))
        self.chart.draw()
        self.lbl_p.setText(f"PASS: {stats[0]} ({(stats[0]/total)*100:.1f}%)")
        self.lbl_f.setText(f"FAIL: {stats[1]} ({(stats[1]/total)*100:.1f}%)")
        self.lbl_w.setText(f"WARNING: {stats[2]} ({(stats[2]/total)*100:.1f}%)")
    ...
    def export_csv(self):
        ...
        if path:
            with open(path, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f); writer.writerow(self.headers)
                for r in range(125): writer.writerow([self.table.item(r, c).text() for c in range(11)])
            QMessageBox.information(self, "Xong", "Đã xuất báo cáo.")

    def toggle_connection(self):
        if self.ser is None:
            try:
                self.ser = serial.Serial(self.combo_ports.currentText(), 9600, timeout=1.5)
                self.ser.write(b"*IDN?\r\n") # Kiểm tra thiết bị [cite: 1810]
                if self.ser.readline():
                    self.btn_connect.setText("NGẮT KẾT NỐI"); self.btn_connect.setStyleSheet("background-color: #d32f2f; color: white;")
                    self.lbl_msg.setText("ĐÃ KẾT NỐI")
                else:
                    self.ser.close(); self.ser = None; QMessageBox.critical(self, "Lỗi", "Máy không phản hồi!")
            except Exception as e: QMessageBox.critical(self, "Lỗi", str(e))
        else:
            self.ser.write(b":SYSTem:LOCAL\r\n") # Thoát chế độ Remote [cite: 342, 349, 1406]
            self.ser.close(); self.ser = None
            self.btn_connect.setText("KẾT NỐI"); self.btn_connect.setStyleSheet("")
